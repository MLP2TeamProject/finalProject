			...
			if (buyBooks[0]) {
				const hasBids = await Promise.all(
					buyBooks.map(async (item) => {
						const [bidList] = await conn.query(
							sql.checkBids,
							item.product_id
						);
				...
				if (hasBids[0][0]) {
					callback({
						status: 200,
						message:
							'구매 희망 도서에 입찰된 내용이 있어 회원 탈퇴가 불가능합니다.',
						data: 'impossible',
					});
				} else {
					...
					if (hasMyBids.length !== 0) {
						...
						console.log('1. 탈퇴 가능');
						// 구매요청한 도서 삭제
						const [myBooks] = await conn.query(sql.deleteBuyBooks, [
							email,
						]);
						console.log('2', myBooks);
						if (myBooks) {
							console.log(
								'구매요청 도서 삭제 완료. 회원탈퇴 가능'
							);
							leaveSite();
						}
					}
