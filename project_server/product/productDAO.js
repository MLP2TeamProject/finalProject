...
const sql = {
	...
	update: "update product set master_price =?, content = ? where product_id = ?",
	//이부분은 buy에서 글쓴이가 수정하려면 쓸 부분
	insertAuction: "INSERT INTO auction (product_id, email, auction_price, picture, product_status) VALUES (?, ?, ?, ?, ?)",
...
const productDAO = {
	...
	update: async (item, callback) => {
		...
		try {
			conn = await getPool().getConnection();
			const [resp] = await conn.query(sql.update, [item.master_price, item.content, item.product_id]);
	...
	bidding: async (data, callback) => {
		...
		try {
			console.log("1", data);
			conn = await getPool().getConnection();
			const [result] = await conn.query(sql.insertAuction, [data.product_id, data.email, data.auction_price, data.picture, data.product_status]);
			if (result) {
				const [bookInfo] = await conn.query(sql.checkBookTitle, [data.product_id]); //insert성공하면 책 정보를 db에서 조회, bookinfo에 결과 할당하기
